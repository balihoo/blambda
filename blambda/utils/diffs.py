...
def needs_update(manifest, sha_from, sha_to, show_diff=False, verbose=False):
    ...
    if ret != 0 and "unknown revision" in "".join(stdout + stderr):
        vprint(pRed("{} needs update because SHA {} is unknown to git\n".format(basename, sha_from)))
        return True
    else:
        ...
        if len(matches) > 0:
            ...
            vprint((pGreen("{}") + pYellow(" needs update because {} has changed between {} and {}")).format(
                basename, names, sha_from, sha_to))
            if show_diff:
